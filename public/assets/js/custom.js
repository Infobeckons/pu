...
jQuery(document).ready(function(){
	...
	if(countsing=='1')
	  ...
	  else
	  {
		...
		jQuery('#amount').val('400');
		 ...
		 {
		 data+='<div class="form-group member-col" style="display:block" id="member'+i+'"><label class="col-md-3 control-label" for="member'+i+'" id="member'+i+'">Member Name '+i+'</label><div class="col-md-3"><input id="membername'+i+'" name="membername'+i+'" type="text" placeholder="Name" class="form-control input-md" required></div><div class="col-md-3 space-left"><input id="memberfathername'+i+'" name="memberfathername'+i+'" type="text" placeholder="Father Name" class="form-control input-md" required></div><div class="col-md-3 space-left"><input id="memberdob'+i+'" name="memberdob'+i+'" type="text" placeholder="Date Of Birth" class="form-control input-md" required></div></div>';
		 }
	...
	if(eventName=='rangolicom'){ 
	$('#rangolimem').css('display','block');
	var amount=100; }
	 else { 
	$('#rangolimem').css('display','none');
	}
	
	if(eventName=='fashioncomp'){ 
	$('#fashioncomshow').css('display','block');
	var amount=300; }
	 else { 
	$('#fashioncomshow').css('display','none');
	}
	...
	jQuery('#rangolimemsel').change(function(){
	  var countmem=$(this).val();
	  var newrangloliamount=parseInt(countmem)*100;
	  jQuery('#amount').val(newrangloliamount);
	...
	jQuery('#fashioncommemsel').change(function(){
	  var countmem=$(this).val();
	  var newfashionamount=parseInt(countmem)*300;
	  jQuery('#amount').val(newfashionamount);
	  if(countmem=='1')
	  {
		   $('#groupname').text('Name');
		 $(".danceapend").html('');
